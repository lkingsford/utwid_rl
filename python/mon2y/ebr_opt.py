...
def most_trusted_hyperrewards(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["ratio"] = ((df["turns"] - df["rwalk"]) / df["turns"]).replace([np.inf, -np.inf], 0)

    if "sum_diff_est_reward" not in df.columns or df["sum_diff_est_reward"].max() == df["sum_diff_est_reward"].min():
        df["trust"] = df["ratio"]
    else:
    ...
    if pd.isna(trust_mu) or pd.isna(trust_sigma):
        return pd.DataFrame()
